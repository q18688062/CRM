...
class Customer(models.Model):
    ...
    def show_status(self):
        get_dic = {'signed': "btn-primary",
                    'unregistered': "btn-danger",
                    'studying': 'btn-warning',
                    'paid_in_full': "btn-success"}

        return mark_safe('<span style="padding:8px" class="{}">{}</span>'.format(get_dic[self.status], self.get_status_display()))
...
class Enrollment(models.Model):
    ...
    def contract_status(self):
        get_dic = {'False': ["btn-danger", "审批中"],
                   'True': ["btn-success", "审批通过"]}
        return mark_safe('<span style="padding:8px" class="{}">{}</span>'.format(get_dic[str(self.contract_approved)][0], get_dic[str(self.contract_approved)][1]))
...
class CourseRecord(models.Model):
    ...
    re_class = models.ForeignKey('ClassList', verbose_name="班级", related_name='cl_record')
    teacher = models.ForeignKey('UserProfile', verbose_name="讲师", related_name='te_record')
