...
from rbac.form import RoleForm, MenuForm, PermissionForm, MultiPermissionForm
...
def permission_change(request, pk=None):
    ...
    if request.method == 'POST':
        form_obj = PermissionForm(instance=obj, data=request.POST)
        if form_obj.is_valid():
            form_obj.save()
            return redirect(reverse('menu_list'))
    return render(request, 'form.html', {'form_obj': form_obj})
...
def distribute_permissions(request):
    ...
    root_permission = models.Permission.objects.filter(menu__isnull=False).values('id', 'url_name', 'menu_id')
    ...
    node_permission = models.Permission.objects.filter(menu__isnull=True).values('id', 'url_name', 'parent_id')
